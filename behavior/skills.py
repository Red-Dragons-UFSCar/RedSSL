...
'''
def attack_ball_fisico(robot, field):
    ...
    angle_ball_to_target = np.arctan2(
        target_y_final - ball_coordinates.Y , target_x_final - ball_coordinates.X
    )

    # Estado A: Ir para a bola
    # Estado B: Controlar angulo
    # Estado C: Empurrar a bola

    approach_offset = 30
    threshold = 15

    if field.atacante_state_real == 'A': 
        # Estado A: Se posicionar atrás da bola
        target_x = ball_coordinates.X - approach_offset * np.cos(angle_ball_to_target)
        target_y = ball_coordinates.Y - approach_offset * np.sin(angle_ball_to_target)
        target_theta = angle_ball_to_target
        field.disable_angular_controller = False

        go_to_point(robot, target_x, target_y, field, target_theta, threshold)

        robot.w = 0

        if robot.target_reached(threshold):
            robot.vx = 0
            robot.vy = 0
            #robot0.w = 0
            ...
            if field.counter_attacker_stop > field.threshold_attacker_stop:
                field.atacante_state_real = "B"
                field.counter_attacker_stop = 0
        else:
            field.counter_attacker_stop = 0

    elif field.atacante_state_real == 'B':
        target_x = ball_coordinates.X - approach_offset * np.cos(angle_ball_to_target)
        target_y = ball_coordinates.Y - approach_offset * np.sin(angle_ball_to_target)
        target_theta = angle_ball_to_target
        go_to_point(robot, target_x, target_y, field, target_theta, threshold)
        ...
        if not robot.target_reached(threshold):
            field.atacante_state_real = "A"
        else:
            field.counter_attacker_stop += 1
            if field.counter_attacker_stop > 2*field.threshold_attacker_stop:
                field.atacante_state_real = "C"
                field.counter_attacker_stop = 0

    elif field.atacante_state_real == 'C':
        target_x = 450
        target_y = 150
        target_theta = angle_ball_to_target
        go_to_point(robot, target_x, target_y, field, target_theta, threshold)
    ...
    print("Estado: ", field.atacante_state_real)
